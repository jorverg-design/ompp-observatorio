...
def upsert_series(series: str, obs_date: str, value: float, source: str):
    con = condb()
    cur = con.cursor()
    cur.execute("DELETE FROM external_series WHERE series=? AND obs_date=?", (series, obs_date))
    cur.execute("""
    INSERT INTO external_series(created_at, series, obs_date, value, source)
    VALUES(?,?,?,?,?)
    """, (datetime.now(timezone.utc).isoformat(), series, obs_date, value, source))
    con.commit()
    con.close()
...
def fetch_usd_pyg():
    ...
    upsert_series("USD_PYG", obs_date, pyg, "open.er-api.com")
